...
- 因为数据库可以实现数据的持久化(persistence)

  > **即把数据保存到可掉电式存储设备中以供之后使用**，大多数情况下，特别是企业级应用，**数据持久化意味着将内存中的数据保存到硬盘上加以“固化”**，而持久化的实现过程大多通过各种关系数据库来完成，我们所谓的内存是使用低电压和高电压来区别0和1，进而进行数据的存储，一旦要是断电了，电压都没有了，就没有0和1了，所以内存中存储的数据一断电就没有了，所以我们需要将内存中的数据进行持久化，在持久化时，我们就需要一些相关的存储介质，比如我们可以将相关数据存储到对应的一些文件中（如.jpg，.java，.doc等文件），我们还可以考虑将更复杂的数据存储到数据库当中，以及其他的一些介质（如xml文件等等），但是除了数据库还有其他的一些介质可以存储数据，为什么非常看重数据库呢？数据库存储数据的特点如下：
  >
  > 1. 可存储的数据量非常庞大：单单一张表就能存储几千万甚至上亿条数据，若存储在文件中，那么查找效率会非常差，而存储在数据库中有很多优化手段可以提高查找效率
  > 2. 相比于文件来说，数据库可存储的数据类型是多种多样的（如字符串、整型、浮点型、日期型、图片、大文本等等）

- 持久化的主要作用是**将内存中的数据存储在关系型数据库中**，当然也可以存储在磁盘文件、XML数据文件中

  ![image-20220618022949478](D:\Martha_SoftwareDatas\Typora\REAR\MySQL\MySQL_images\image-20220618022949478.png)
...
- 它是一种操纵和管理数据库的大型软件，用于建立、使用和维护数据库，对数据库进行统一管理和控制。用户通过DBMS访问数据库中表内的数据，我们平常所说的安装MySQL数据库，其实就是在装这个DBMS数据库管理软件，安装完DBMS之后，就可以通过它去操作数据库了
  
- **SQL：结构化查询语言（Structured Query Language）**
  
  - 通过编写SQL语言，将其运行在DBMS系统中，使得DBMS帮助我们去数据库中进行相关的操作，是专门用来与数据库通信的语言，即**DBMS系统通过执行SQL语言来操作数据库**
  
  > Tips：Windows系统中MySQL的真实数据文件一般保存在`C:/ProgramData/MySQL/MySQL Server 8.0/Data/`下，该目录下的每一个文件夹代表一个数据库，每一个文件夹下就是具体的表(.ibd文件)，我们所说的数据库就是它，而数据库管理系统是我们安装的软件MySQL Server 8.0，我们就是通过DBMS来操作这些.ibd文件的，.ibd文件不能直接双击打开

##### 2.2 数据库与数据库管理系统的关系

> 数据库管理系统（DBMS）可以管理多个数据库（DB），一般开发人员会针对每一个应用创建一个数据库，为了保存应用中的实体数据，一般会在数据库中创建多个表，以保存实体用户的数据，数据库管理系统、数据库和表的关系如下图所示：由Web服务器发送指令给到MySQL数据库管理系统，然后DBMS根据执行相关SQL语句，将所需数据返回Web服务器

![image-20220618024244924](D:\Martha_SoftwareDatas\Typora\REAR\MySQL\MySQL_images\image-20220618024244924.png)

![image-20220618024925677](D:\Martha_SoftwareDatas\Typora\REAR\MySQL\MySQL_images\image-20220618024925677.png)

#### 3.MySQL介绍

+ MySQL是一个**开源**的**关系型**数据库管理系统
+ MySQL6.x版本之后分为了`社区版`（免费）和`商业版`（付费）
+ MySQL可以定制，采用了GPL协议，我们可以通过修改MySQL源代码开发自己的MySQL系统
+ MySQL支持大型数据库，可以处理拥有上千万条记录的大型数据库，但是一般情况下，记录上千万条我们就要考虑分库分表了，以便更好地实现数据的查询操作
+ MySQL使用`基本标准的SQL语言`形式，也不是非常标准，比如像检查约束，MySQL也没有支持，不同的数据库支持SQL语言的标准是有区别的

#### 4. RDBMS和非RDBMS

##### 4.1 关系型数据库(RDBMS)

> 关系型数据库（RDBMS）绝对是DBMS的主流，其中使用最多的DBMS分别就是Oracle、MySQL、SQL Server，这些都是关系型数据库（RDBMS）

######  4.1.1 实质

- 关系型的数据库是**最古老**的数据库类型，关系型数据库模型是把复杂的数据结构归结为简单的**二元关系** （即二维表格形式），如下：

  ![image-20220618184951465](D:\Martha_SoftwareDatas\Typora\REAR\MySQL\MySQL_images\image-20220618184951465.png)
...
- **便于复杂查询**可以用SQL语句方便的在一个表或多个表之间做非常复杂的数据查询。
- **事务支持**使得对于安全性能很高的数据访问要求得以实现。

##### 4.2 非关系型数据库(非RDBMS)

> 如MongoDB是文档型数据库，Redis是key-value对数据库，ElasticSearch是基于搜索引擎的数据库，这些都是非关系型数据库

**非关系型数据库**，可看成传统关系型数据库的功能**阉割（轻量级）版本**（比如舍去了复杂查询特性），基于键值对存储数据，不需要经过SQL层的解析，**性能非常高**。同时，通过减少不常用的功能，进一步提高性能

###### 4.2.1 有哪些非关系型数据库

> 相比于SQL，NoSQL泛指非关系型数据库，包括键值型数据库、文档型数据库、搜索引擎和列存储等，除此之外还包括图形数据库，也只有用NoSQL一词才能将这些技术囊括进来

+ 键值型数据库

  > 键值对型数据库通过key-value键值对的方式来存储数据，其中key和value可以是简单的对象，也可以是复杂的对象。Key对位唯一的标识符，优点就是查找速度快，在这方面明显优于关系型数据库，缺点就是无法像关系型数据库一样使用条件过滤（比如WHERE条件），如果不知道要去哪儿查找数据，就需要遍历所有的key，会消耗大量的计算时间。
  >
  > 键值型数据库典型的使用场景就是作为`内存缓存`。Redis是最流行的键值型数据库，像秒杀等等一些场景就需要使用到Redis

+ 文档型数据库

  》 文档型数据库可存放并获取文档，可以是XML、JSON等格式，在数据库中文档作为处理信息的基本单位，一个文档就相当于是一条记录，文档数据库所存放的文档，就相当于键值对数据库所存放的值，MongoDB是最流行的文档型数据库，此外还有CouchDB等
  
+ 搜索引擎数据库

  》 虽然关系型数据库采用了**索引提升检索效率**，但是针对**全文索引效率缺比较低**，搜索引擎数据库是应用在搜索引擎领域的数据存储形式，由于搜索引擎会爬取大量的数据，并以特定的格式进行存储，这样在检索的时候才能保证性能最优，核心原理是倒排索引
  》
  》 典型产品就是Solr、Elasticsearch、Splunk等
  》
  》 目前针对数据量比较大的我们都是使用搜索引擎数据库

+ 列式数据库

  》 列式数据库是相对于行式存储的数据库，Oracle、MySQL、SQL Server等数据库都是采用行式存储（Row-based），而列式数据库按照列存储到数据库中，这样做的好处就是可以大大降低系统的IO，适合分布式文件系统，不足在于功能相对有限，典型产品就是HBase等，为什么能降低IO操作呢？就是因为在行式存储中，如果我们要读取一行中某个字段，在查询到该行记录时，会将整行记录中所有字段都加载进内存中，也就是不想获取的字段也加载进来了，内存中存入了7个字段，结果实际上只需要一个读取出来，这样就浪费了内存空间，所以**采用行式存储，那么冗余字段就会过多，也就意味着更多的IO操作，性能肯定就不会高；所以这就是为什么提出列式存储，我就只查询你想要的那一列即可，其他的字段就不往内存中加载了，自然IO次数就少了，所以速度就快了**

+ 图形数据库

  》 图形数据库，利用了图这种数据结构存储了实体（对象）之间的关系，图形数据库最典型的例子就是人与人之间的关系，数据模型主要就是以结点和边（关系）来实现，特点在于能够高效地解决复杂的关系问题。
  》
  》 图形数据库顾名思义就是一种存储图形关系的数据库，它利用了图的数据结构存储了实体对象之间的关系，关系型数据用于存储明确关系的数据，但对于复杂关系的数据存储却力不从心，如社交网络任务之间的关系，如果使用关系型数据库则非常复杂，用图形数据库将变得非常简单，典型产品就是Neo4J、InfoGrid等

  ![image-20220702154808433](D:\Martha_SoftwareDatas\Typora\REAR\MySQL\MySQL_images\image-20220702154808433.png)

###### NoSQL的演变

》 我们的SQL主要用于操作RDBMS，而NoSQL主要用于操作非RDBMS，所以由于SQL一直霸榜DBMS，因此许多人思考能否有一种数据库技术能远离SQL，于是NoSQL诞生了，但随着发展越来越离不开SQL，到目前为止NoSQL阵营中的DBMS都会有实现类似SQL的功能，NoSQL对SQL做出了很好的补充，比如实际开发中，有很多业务需求，其实并不需要完整的关系型数据库功能，非关系型数据库的功能就足够使用了，这种情况下，使用性能更高，成本更低的非关系型数据库当然是更明智的选择，比如日志收集、排行榜、定时器等
...
- 必须保证你的字段没有和保留字、数据库系统或常用方法名等关键字冲突。如果坚持使用，请在SQL语句中使用`（着重号）引起来
